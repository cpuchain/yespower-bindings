...
import { yespower, yespower_async } from '../index.js';

// ---------------------------------------------------------------------------
// Reference vectors from yespower-c/TESTS-OK, reproduced exactly the way the C
// test driver (yespower-c/tests.c) does.
//
// The driver hashes an 80-byte input `src` where src[i] = (i * 3) & 0xff, with
// a version (5 = YESPOWER_0_5, 10 = YESPOWER_1_0), N, r and optional personality.
// The "BSTY" case uses the 80-byte src itself as the (binary) personality.
// Two extra lines XOR every digest produced while looping N=1024,2048,4096 and
// r=8..32, using a variant src where src[0] = 43.
// ---------------------------------------------------------------------------

// Replicates `src` in tests.c: src[i] = i * 3 (mod 256). 80 bytes.
function refSrc(): Buffer {
    const src = Buffer.alloc(80);
...
// All 16 single-hash lines from TESTS-OK (the two XOR lines are checked below).
...
// XOR aggregate lines from TESTS-OK.
...
function hashCase(c: RefCase): Buffer {
    const pers = c.pers === BSTY ? SRC : c.pers === null ? undefined : c.pers;
    return yespower(SRC, c.N, c.r, pers, c.version);
}

// Reproduces print_yespower_loop(): src[0] = 43, XOR all digests over the grid.
function xorLoop(version: 5 | 10, pers: string | null): Buffer {
    const src = refSrc();
    src[0] = 43;
    const xor = Buffer.alloc(32);
    for (let N = 1024; N <= 4096; N <<= 1) {
        for (let r = 8; r <= 32; r++) {
            const dst = yespower(src, N, r, pers ?? undefined, version);
            for (let i = 0; i < 32; i++) {
                xor[i] ^= dst[i];
            }
        }
...
describe('Yespower (NAPI) — yespower-c/TESTS-OK reference vectors', () => {
    it('single-hash vectors (v0.5 + v1.0, incl. binary BSTY pers)', () => {
        for (const c of refCases) {
            const hashed = hashCase(c).toString('hex');
            assert.strictEqual(hashed, c.output, `yespower(${c.version}, ${c.N}, ${c.r}, ${String(c.pers)})`);
        }
    });

    it('XOR of yespower(5, ...) aggregate', () => {
        assert.strictEqual(xorLoop(5, 'Client Key').toString('hex'), xorV5);
    });

    it('XOR of yespower(10, ...) aggregate', () => {
        assert.strictEqual(xorLoop(10, null).toString('hex'), xorV10);
    });
});
...
describe('Yespower (NAPI) — additional fixed-input vectors', () => {
    it('Test Cases', () => {
...
// ---------------------------------------------------------------------------
// Asynchronous variant (yespower_async). Runs on the libuv threadpool and
// returns a Promise; results must match the synchronous function exactly.
// ---------------------------------------------------------------------------

describe('Yespower (NAPI async) — yespower_async', () => {
    it('matches the TESTS-OK reference vectors', async () => {
        for (const c of refCases) {
            const pers = c.pers === BSTY ? SRC : c.pers === null ? undefined : c.pers;
            const hashed = await yespower_async(SRC, c.N, c.r, pers, c.version);
            assert.strictEqual(
                hashed.toString('hex'),
                c.output,
                `yespower_async(${c.version}, ${c.N}, ${c.r}, ${String(c.pers)})`,
            );
        }
    });

    it('runs concurrently and matches the synchronous result', async () => {
        const inputs = Array.from({ length: 8 }, (_, i) => {
            const b = Buffer.alloc(4);
            b.writeUInt32BE(i);
            return b;
        });
        const expected = inputs.map((b) => yespower(b, 1024, 8).toString('hex'));
        const actual = await Promise.all(inputs.map((b) => yespower_async(b, 1024, 8)));
        for (let i = 0; i < inputs.length; i++) {
            assert.strictEqual(actual[i].toString('hex'), expected[i]);
        }
    });

    it('rejects on an invalid version', async () => {
        await assert.rejects(
            // @ts-expect-error intentionally invalid version
            () => yespower_async(SRC, 2048, 8, undefined, 7),
            /version/,
        );
    });
});
